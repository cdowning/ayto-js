$(function() {
    ...
    function postUpdate() {
        ...
        var transition = progressBar.css('transition');
        ...
        if (worker === null) {
            ...
            worker.addEventListener('message', function(e) {
                if ('progress' in e.data) {
                    progressBar.css('transition', transition);
                ...
                for (var y = 0; y < CARDINALITY; y++) {
                    yMax[y] = Math.max.apply(null, results.map(function(row) {
                        return row[y]
                    }));
                }
    ...
    women.forEach(function(woman) {
        ...
        men.forEach(function(man) {
            ...
            cell.click(function(event) {
                ...
                if (cell.hasClass('forced-non-match'))
                    ga('send', 'event', man + '-' + woman, 'nonmatch', '', 0, {
                        'nonInteraction': 1
                    });
                if (cell.hasClass('forced-match'))
                    ga('send', 'event', man + '-' + woman, 'match', '', 0, {
                        'nonInteraction': 1
                    });
            });
        });
    ...
    updateFromSlider({
        value: {
            newValue: truthBooths.length + matchUps.length
        }
    });
});
